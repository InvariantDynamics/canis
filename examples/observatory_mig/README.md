# Kepler + Observatory + MIG (Platform-First Interop)

This example package treats Observatory + MIG as the control-plane foundation and exposes **Kepler** as a provider (`provider_id=kepler`) on the same contract as partner AI-SRE vendors.

## What this includes

- [`holmes-values.yaml`](./holmes-values.yaml): Helm values to run Holmes base with Kepler-facing MIG patterns
- [`toolsets-cli.yaml`](./toolsets-cli.yaml): CLI profile for reading Observatory + MIG data paths
- [`mig-cli.sh`](./mig-cli.sh): Terminal helper for MIG and agent lifecycle APIs
- [`observatory-mig-contract.yaml`](./observatory-mig-contract.yaml): Machine-readable partner contract
...
Core Observatory reads (for context ingestion):

- `GET /v1/mig/status`
- `GET /v1/observatory/entities`
- `GET /v1/observatory/edges`
...
export OBSERVATORY_TOKEN="${OBSERVATORY_TOKEN:-observatory-dev-admin}"

kubectl create secret generic holmes-mig-secrets \
  -n observatory-platform \
  --from-literal=anthropic-api-key="${ANTHROPIC_API_KEY}" \
  --from-literal=observatory-token="${OBSERVATORY_TOKEN}" \
...
helm upgrade --install holmes-mig robusta/holmes \
  -n observatory-platform \
  --create-namespace \
  -f examples/observatory_mig/holmes-values.yaml
```

## 3) Validate service and run Kepler evaluation

```bash
kubectl rollout status deploy/holmes-mig-holmes -n observatory-platform
kubectl port-forward svc/holmes-mig-holmes 18080:80 -n observatory-platform
```
...
```bash
chmod +x examples/observatory_mig/mig-cli.sh
...
- Execution authority remains in Observatory control plane.
- Approval channel is Observatory UI.
...
export KEPLER_NATS_URL="nats://nats.observatory-platform.svc.cluster.local:4222"
