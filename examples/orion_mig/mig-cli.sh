...
TOKEN="${ORION_TOKEN:-orion-dev-admin}"
...
Environment:
  ...
  ORION_TOKEN      Bearer token (default: orion-dev-admin)
...
case "${cmd}" in
  status)
    ...
    ;;
  entities)
    request "${READ_BASE_URL}" GET "/v1/orion/entities" | pretty_print
    ;;
  edges)
    request "${READ_BASE_URL}" GET "/v1/orion/edges" | pretty_print
    ;;
  incidents)
