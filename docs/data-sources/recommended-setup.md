...
After installing Canis and running your first investigation, connect your data sources so Canis can perform deeper investigations.

## How Canis Works

Canis is an AI troubleshooting agent that investigates issues by pulling data from your existing observability stack. The more data sources you connect, the more thoroughly Canis can investigate — correlating metrics with logs, tracing infrastructure changes to application failures, and building a complete picture of what went wrong.

Canis works across cloud, on-premise, and hybrid environments. If you use Kubernetes, the Kubernetes toolsets are enabled automatically. Everything else — metrics, logs, cloud APIs, databases — is configured through toolsets you enable based on your stack.

## 1. Connect a Metrics Provider

Metrics give Canis visibility into trends over time. Without metrics, Canis can still investigate using logs and infrastructure state, but it won't be able to spot gradual degradation or correlate historical information as well. Metrics are also critical to answering numerical questions, like 'what is the error rate for service xyz?'
...
If you use Grafana, connecting the dashboards toolset lets Canis see what you're already monitoring — it can find relevant dashboards, extract PromQL queries from panels, and use them during investigations.
...
canis ask "what is the health of my environment?"
```

## Next Steps

- **[Interactive Mode](../walkthrough/interactive-mode.md)** - Use Canis interactively for follow-up questions
