...
Canis is an AI agent purpose-built for production observability and incident response.

## 1. Petabyte-Scale Observability Data

Production systems generate enormous amounts of telemetry data. Canis is designed to work at this scale without pulling unbounded data into context:
...
- **Traversable JSON trees**: For APIs that return large JSON payloads, Canis transforms responses into traversable trees with filtering and depth-limiting controls so the LLM can extract data without pulling the entire payload into context
- **Summarization transformers**: For tools that still return large outputs, Canis supports transformers that summarize data before it reaches the LLM

## 2. Operator Mode

Canis can run in the background to proactively find problems and notify your team.

The [Canis Operator](operator/index.md) manages health checks as Kubernetes-native resources:
...
Canis ships with read-only integrations for every major observability vendor. Connect custom MCP servers for proprietary tools, or use the [HTTP connector](data-sources/api-toolsets.md) to turn any REST API into an LLM-friendly data source through YAML alone.
...
Instead of every engineer connecting their local AI tools to production with personal credentials that carry write access, deploy one Canis instance with scoped, read-only access. Let engineers use LLMs with observability data - safely.
...
Canis automatically transforms these raw endpoints to be LLM-friendly:
...
Canis infers service relationships from the telemetry data already flowing through your stack:
...
Canis can integrate into your existing workflows, by automatically fetching alerts and incidents from AlertManager, PagerDuty, OpsGenie, or more—and writing the investigation results back to the source.

### Alert Source Integration

Canis fetches alerts directly from your incident management systems:

```bash
# Investigate Prometheus/AlertManager alerts
canis investigate alertmanager --alertmanager-url http://alertmanager:9093

# Investigate PagerDuty incidents
canis investigate pagerduty --pagerduty-api-key <key>

# Investigate OpsGenie alerts
canis investigate opsgenie --opsgenie-api-key <key>

# Investigate Jira tickets
canis investigate jira --jira-url https://company.atlassian.net \
  --jira-username user@example.com --jira-api-key <key>

# Investigate GitHub issues
canis investigate github --github-owner org --github-repository repo \
  --github-pat <token>
```

Canis automatically extracts alert metadata (labels, severity, annotations), selects relevant toolsets, and begins investigation.

### Results Delivery

Canis can write investigation findings back to the source system:

```bash
# Write findings back to PagerDuty incident
canis investigate pagerduty --pagerduty-api-key <key> --update
...
See the [Installation Guide](installation/cli-installation.md) to set up Canis.
