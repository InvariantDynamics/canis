...
Destinations are used when a HealthCheck or ScheduledHealthCheck has `mode: alert`. When a check fails or returns an error, the Canis Operator sends notifications to all configured destinations.
...
Canis Operator currently supports two alert destination types:
...
Configure the Slack token globally for all checks (On Canis, not Operator):
...
![Example Slack alert generated by Canis Operator](../assets/operator-alert-example.png)
...
PagerDuty incidents created by Canis include:

**Basic Information:**
- **Summary**: `Canis Check Failed: <check-name>`
